...
def dig_pow(n, p):
    #print(n,p,[str(x)+"^"+str(p+i) for i,x in enumerate(str(n))],sum(pow(int(x),p+i) for i,x in enumerate(str(n)))/n)
    check = lambda x:int(x) if int(x)==x else -1
    return check(sum(pow(int(x),p+i) for i,x in enumerate(str(n)))/n) or -1

def clever_dig_pow(n, p):
    k, fail = divmod(sum(int(d)**(p + i) for i, d in enumerate(str(n))), n)
    return -1 if fail else k

print(dig_pow(71980,5), -1)
